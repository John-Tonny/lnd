...
func (m *mockConstraints) ChannelBudget(chans []LocalChannel,
	balance vclutil.Amount) (vclutil.Amount, uint32) {
...
func (m *mockConstraints) MinChanSize() vclutil.Amount {
	return 1e7
}
func (m *mockConstraints) MaxChanSize() vclutil.Amount {
	return 1e8
}
...
func setup(t *testing.T, initialChans []LocalChannel) (*testContext, func()) {
	...
	var availableFunds vclutil.Amount = 10 * vclutil.SatoshiPerBitcoin
...
func TestAgentChannelOpenSignal(t *testing.T) {
	...
	newChan := LocalChannel{
		ChanID:  randChanID(),
		Balance: vclutil.SatoshiPerBitcoin,
	}
...
func TestAgentHeuristicUpdateSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: 1,
			amt:     1 * vclutil.SatoshiPerBitcoin,
		},
	)
...
func TestAgentChannelFailureSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx, moreChansResp{1, 5 * vclutil.SatoshiPerBitcoin})
...
func TestAgentChannelCloseSignal(t *testing.T) {
	...
	initialChans := []LocalChannel{
		...
		{
			ChanID:  randChanID(),
			Balance: vclutil.SatoshiPerBitcoin * 2,
		},
	}
...
func TestAgentBalanceUpdate(t *testing.T) {
	...
	testCtx.Lock()
	testCtx.walletBalance += vclutil.SatoshiPerBitcoin * 5
	testCtx.Unlock()
...
func TestAgentImmediateAttach(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: numChans,
			amt:     5 * vclutil.SatoshiPerBitcoin,
		},
	)
	...
	for i := 0; i < numChans; i++ {
		...
		case openChan := <-chanController.openChanSignals:
			if openChan.amt != vclutil.SatoshiPerBitcoin {
				t.Fatalf("invalid chan amt: expected %v, got %v",
					vclutil.SatoshiPerBitcoin, openChan.amt)
			}
...
func TestAgentPendingChannelState(t *testing.T) {
	...
	// heuristic.
	testCtx.Lock()
	testCtx.walletBalance += 0.4 * vclutil.SatoshiPerBitcoin
	testCtx.Unlock()
	...
	// channels.
	select {
	case testCtx.constraints.moreChansResps <- moreChansResp{1, vclutil.SatoshiPerBitcoin}:
	case <-time.After(time.Second * 10):
		t.Fatalf("need more chans wasn't queried in time")
	}
...
func respondWithScores(t *testing.T, testCtx *testContext,
	channelBudget vclutil.Amount, existingChans, newChans int,
...
func checkChannelOpens(t *testing.T, testCtx *testContext,
	allocation vclutil.Amount, numChans int) []NodeID {

	var nodes []NodeID

	// The agent should attempt to open channels, totaling what we expect.
	var totalAllocation vclutil.Amount
	chanController := testCtx.chanController.(*mockChanController)
	for i := 0; i < numChans; i++ {
		select {
		case openChan := <-chanController.openChanSignals:
...
func TestAgentChannelSizeAllocation(t *testing.T) {
	...
	var channelBudget vclutil.Amount = 5 * vclutil.SatoshiPerBitcoin
	...
	// channels.
	expectedAllocation := testCtx.constraints.MaxChanSize() * vclutil.Amount(numNewChannels)
	...
	channelBudget = vclutil.SatoshiPerBitcoin * 3 / 2
	...
	// our budget.
	channelBudget = vclutil.SatoshiPerBitcoin * 5
	numNewChannels = 2
	respondWithScores(
		t, testCtx, channelBudget, numExistingChannels,
		numNewChannels, nodeScores,
