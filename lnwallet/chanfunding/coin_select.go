...
type ErrInsufficientFunds struct {
	amountAvailable vclutil.Amount
	amountSelected  vclutil.Amount
}
...
// change+fees.
func selectInputs(amt vclutil.Amount, coins []Coin) (vclutil.Amount, []Coin, error) {
	satSelected := vclutil.Amount(0)
	for i, coin := range coins {
		satSelected += vclutil.Amount(coin.Value)
		if satSelected >= amt {
...
// function properly.
func CoinSelect(feeRate chainfee.SatPerKWeight, amt vclutil.Amount,
	coins []Coin) ([]Coin, vclutil.Amount, error) {
...
func CoinSelectSubtractFees(feeRate chainfee.SatPerKWeight, amt,
	dustLimit vclutil.Amount, coins []Coin) ([]Coin, vclutil.Amount,
