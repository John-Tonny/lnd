...
func testAddSettleWorkflow(t *testing.T, tweakless bool) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(vclutil.SatoshiPerBitcoin)
	...
	mSatTransferred := lnwire.NewMSatFromSatoshis(vclutil.SatoshiPerBitcoin)
...
func TestCheckCommitTxSize(t *testing.T) {
	...
	checkSize := func(channel *LightningChannel, count int) {
		...
		actualCost := blockchain.GetTransactionWeight(vclutil.NewTx(commitTx))
...
func TestHTLCDustLimit(t *testing.T) {
	...
	// Bob's dust limit.
	htlcSat := (vclutil.Amount(500) + HtlcTimeoutFee(
...
func TestHTLCSigNumber(t *testing.T) {
	...
	createChanWithHTLC := func(htlcValues ...vclutil.Amount) (
	...
	belowDust := vclutil.Amount(500) + HtlcTimeoutFee(
		channeldb.SingleFunderTweaklessBit, feePerKw,
	)
	aboveDust := vclutil.Amount(1400) + HtlcSuccessFee(
		channeldb.SingleFunderTweaklessBit, feePerKw,
	)
...
func TestCancelHTLC(t *testing.T) {
	...
	// of the new HTLC.
	aliceExpectedBalance := vclutil.Amount(vclutil.SatoshiPerBitcoin*4) -
	...
	expectedBalance := vclutil.Amount(vclutil.SatoshiPerBitcoin * 5)
...
func TestCooperativeCloseDustAdherence(t *testing.T) {
	...
	setDustLimit := func(dustVal vclutil.Amount) {
	...
	// satoshis.
	dustLimit := vclutil.Amount(10000)
	...
	aliceFee := vclutil.Amount(aliceChannel.CalcFee(aliceFeeRate)) + 1000
	aliceSig, _, _, err := aliceChannel.CreateCloseProposal(
		aliceFee, aliceDeliveryScript, bobDeliveryScript,
	)
	if err != nil {
	...
	bobFee := vclutil.Amount(bobChannel.CalcFee(bobFeeRate)) + 1000
	bobSig, _, _, err := bobChannel.CreateCloseProposal(
		bobFee, bobDeliveryScript, aliceDeliveryScript,
	)
	if err != nil {
	...
	aliceBal := lnwire.NewMSatFromSatoshis(vclutil.SatoshiPerBitcoin)
...
func TestAddHTLCNegativeBalance(t *testing.T) {
	...
	htlcAmt = lnwire.NewMSatFromSatoshis(2 * vclutil.SatoshiPerBitcoin)
...
func TestFeeUpdateOldDiskFormat(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(0.1 * vclutil.SatoshiPerBitcoin)
	const numHTLCs = 30
...
func TestChanAvailableBalanceNearHtlcFee(t *testing.T) {
	...
	aliceBalance := lnwire.NewMSatFromSatoshis(5 * vclutil.SatoshiPerBitcoin)
	bobBalance := lnwire.NewMSatFromSatoshis(5 * vclutil.SatoshiPerBitcoin)
...
func TestDesyncHTLCs(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(4.1 * vclutil.SatoshiPerBitcoin)
	htlc, _ := createHTLC(0, htlcAmt)
	...
	htlcAmt = lnwire.NewMSatFromSatoshis(1 * vclutil.SatoshiPerBitcoin)
...
func TestMaxPendingAmount(t *testing.T) {
	...
	maxPending := lnwire.NewMSatFromSatoshis(vclutil.SatoshiPerBitcoin * 3)
	...
	const numHTLCs = 2
	htlcAmt := lnwire.NewMSatFromSatoshis(1.5 * vclutil.SatoshiPerBitcoin)
	for i := 0; i < numHTLCs; i++ {
		htlc, _ := createHTLC(i, htlcAmt)
		if _, err := aliceChannel.AddHTLC(htlc, nil); err != nil {
			t.Fatalf("unable to add htlc: %v", err)
...
func assertChannelBalances(t *testing.T, alice, bob *LightningChannel,
	aliceBalance, bobBalance vclutil.Amount) {
...
func TestChanReserve(t *testing.T) {
	...
	setupChannels := func() (*LightningChannel, *LightningChannel, func()) {
		...
		aliceMinReserve := vclutil.Amount(0.5 *
			vclutil.SatoshiPerBitcoin)
		...
		bobMinReserve := vclutil.Amount(6 * vclutil.SatoshiPerBitcoin)
	...
	//	Alice:	4.5
	//	Bob:	5.0
	htlcAmt := lnwire.NewMSatFromSatoshis(0.5 * vclutil.SatoshiPerBitcoin)
	...
	assertChannelBalances(
		...
		vclutil.SatoshiPerBitcoin*4.5-commitFee, vclutil.SatoshiPerBitcoin*5,
	)
	...
	//	Alice:	1.5
	//	Bob:	9.5
	htlcAmt = lnwire.NewMSatFromSatoshis(3.5 * vclutil.SatoshiPerBitcoin)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		vclutil.SatoshiPerBitcoin*3-commitFee, vclutil.SatoshiPerBitcoin*5,
	)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		vclutil.SatoshiPerBitcoin*3-commitFee, vclutil.SatoshiPerBitcoin*7,
	)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		vclutil.SatoshiPerBitcoin*3-commitFee, vclutil.SatoshiPerBitcoin*6,
	)
}
...
func TestChanReserveRemoteInitiator(t *testing.T) {
	...
	commitFee := aliceChannel.channelState.LocalCommitment.CommitFee
	aliceMinReserve := 5*vclutil.SatoshiPerBitcoin - commitFee
...
func TestChanReserveLocalInitiatorDustHtlc(t *testing.T) {
	...
	commitFee := aliceChannel.channelState.LocalCommitment.CommitFee
	aliceMinReserve := 5*vclutil.SatoshiPerBitcoin - commitFee - htlcSat
...
func TestMinHTLC(t *testing.T) {
	...
	minValue := lnwire.NewMSatFromSatoshis(0.1 * vclutil.SatoshiPerBitcoin)
