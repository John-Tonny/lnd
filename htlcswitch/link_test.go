...
func TestChannelLinkRevThenSig(t *testing.T) {
	...
	const chanAmt = vclutil.SatoshiPerBitcoin * 5
	const chanReserve = vclutil.SatoshiPerBitcoin * 1
...
func TestChannelLinkSingleHopPayment(t *testing.T) {
	...
	alice, bob, cleanUp, err := createTwoClusterChannels(
		vclutil.SatoshiPerBitcoin*3,
		vclutil.SatoshiPerBitcoin*5)
	if err != nil {
	...
	amount := lnwire.NewMSatFromSatoshis(vclutil.SatoshiPerBitcoin)
...
func TestExitNodeTimelockPayloadMismatch(t *testing.T) {
	...
	const amount = vclutil.SatoshiPerBitcoin
...
func TestLinkForwardFeePolicyMismatch(t *testing.T) {
	...
	amountNoFee := lnwire.NewMSatFromSatoshis(vclutil.SatoshiPerBitcoin)
...
func TestLinkForwardMaxHTLCPolicyMismatch(t *testing.T) {
	...
	channels, cleanUp, _, err := createClusterChannels(
		vclutil.SatoshiPerBitcoin*5, vclutil.SatoshiPerBitcoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
...
func TestChannelLinkMultiHopInsufficientPayment(t *testing.T) {
	...
	// rejected.
	amount := lnwire.NewMSatFromSatoshis(4 * vclutil.SatoshiPerBitcoin)
	htlcAmt, totalTimelock, hops := generateHops(amount, testStartingHeight,
		n.firstBobChannelLink, n.carolChannelLink)
...
func newSingleLinkTestHarness(chanAmt, chanReserve vclutil.Amount) (
...
func TestChannelLinkTrimCircuitsPending(t *testing.T) {
	...
	const (
		chanAmt   = vclutil.SatoshiPerBitcoin * 5
		numHtlcs  = 4
...
func TestChannelLinkBandwidthChanReserve(t *testing.T) {
	...
	// a switch initiated payment.  The resulting bandwidth should
	// now be decremented to reflect the new HTLC.
	htlcAmt := lnwire.NewMSatFromSatoshis(3 * vclutil.SatoshiPerBitcoin)
	...
	const bobChanAmt = vclutil.SatoshiPerBitcoin * 1
	const bobChanReserve = vclutil.SatoshiPerBitcoin * 1.5
...
func TestChannelLinkUpdateCommitFee(t *testing.T) {
	...
	// for this test.
	const aliceInitialBalance = vclutil.SatoshiPerBitcoin * 3
	channels, cleanUp, _, err := createClusterChannels(
		aliceInitialBalance, vclutil.SatoshiPerBitcoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
